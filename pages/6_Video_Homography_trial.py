...
st.set_page_config(page_title="Module 3 Trial: Feature Homography & Tracking", layout="wide")
render_home_button()
...
if "video_feature_points" not in st.session_state:
    st.session_state.video_feature_points = []
if "floorplan_feature_points" not in st.session_state:
    st.session_state.floorplan_feature_points = []
...
with tab_region:
    ...
    for key, default in [
        ("four_corners", []),
        ("video_feature_points", []),
        ("floorplan_feature_points", []),
        ("frame_sketches", []),
    ...
    if (
        ...
        if raw_frame_rgb is not None:
            ...
            st.session_state.calibration_frame_width = img_w
            st.session_state.calibration_frame_height = img_h
            ...
            feature_fig.add_trace(
                go.Scatter(
                    x=feature_xx.flatten(),
                    y=feature_yy.flatten(),
                    mode="markers",
                    marker=dict(size=12, color="rgba(0,0,0,0.001)"),
                    hoverinfo="x+y",
                    showlegend=False,
                    name="video_feature_grid",
                )
            )
            selected_video_features = st.session_state.video_feature_points
            if selected_video_features:
                feature_fig.add_trace(
                    go.Scatter(
                        x=[point[0] for point in selected_video_features],
                        y=[point[1] for point in selected_video_features],
                        mode="markers+text",
                        marker=dict(size=14, color="#FFD700", line=dict(color="#000000", width=1.5)),
                        text=[f"F{i + 1}" for i in range(len(selected_video_features))],
            ...
            video_feature_events = st.plotly_chart(
                feature_fig,
                use_container_width=True,
                on_select="rerun",
                selection_mode="points",
                key="video_feature_canvas",
            )
            if video_feature_events and "selection" in video_feature_events:
                ...
                if event_points and len(st.session_state.video_feature_points) < 3:
                    ...
                    if click_hash != st.session_state.get("last_feature_click_hash"):
    ...
    with col_controls:
        st.markdown("#### Feature Point Settings")
        ...
        with col_btn1:
            if st.button(" Clear All Features", use_container_width=True):
                ...
                st.session_state.floorplan_feature_points = []
                st.session_state.four_corners = []
                st.session_state.last_feature_click_hash = None
                st.rerun()
        ...
        for idx in range(max(video_count, floorplan_count)):
            video_point = st.session_state.video_feature_points[idx] if idx < video_count else None
            floorplan_point = st.session_state.floorplan_feature_points[idx] if idx < floorplan_count else None
            video_text = f"({video_point[0]:.2f}, {video_point[1]:.2f})" if video_point else "pending"
            floorplan_text = f"({floorplan_point[0]:.2f}, {floorplan_point[1]:.2f})" if floorplan_point else "pending"
            st.markdown(f"**F{idx + 1}**  Video: `{video_text}`  Floorplan: `{floorplan_text}`")
    ...
    with col_plot:
        ...
        feature_x = [point[0] for point in st.session_state.floorplan_feature_points]
        feature_y = [point[1] for point in st.session_state.floorplan_feature_points]
        corner_x = [point[0] for point in st.session_state.four_corners]
        corner_y = [point[1] for point in st.session_state.four_corners]
        extent_x = all_x + feature_x + corner_x
        extent_y = all_y + feature_y + corner_y
        ...
        if st.session_state.floorplan_feature_points:
            fig.add_trace(
                go.Scatter(
                    x=feature_x,
                    y=feature_y,
                    mode="markers+text",
                    marker=dict(size=14, color="#FFD700", line=dict(color="#000000", width=1.5)),
                    text=[f"F{i + 1}" for i in range(len(feature_x))],
                    textposition="top right",
                    name="Floorplan Features",
                )
            )

        # --- DERIVED ROI CORNERS & POLYGON ---
        ...
        if chart_events and "selection" in chart_events:
            ...
            if (
                ...
                click_hash = f"floorplan_{click_x:.2f}_{click_y:.2f}"

                if click_hash != st.session_state.get("last_feature_click_hash"):
                    st.session_state.last_feature_click_hash = click_hash
                    st.session_state.floorplan_feature_points.append([click_x, click_y])
                    derived_corners = derive_video_corner_mapping(
                        st.session_state.video_feature_points,
                        st.session_state.floorplan_feature_points,
                        st.session_state.get("calibration_frame_width", 0),
                        st.session_state.get("calibration_frame_height", 0),
                    )
